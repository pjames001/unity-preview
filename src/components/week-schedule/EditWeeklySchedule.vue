...
// Initialize the refs with prop data
const scheduleName = ref(props.scheduleData.name);
const weeklySchedule = ref(JSON.parse(JSON.stringify(props.scheduleData.weeklySchedule)));
...
const selectedEmployees = ref(props.scheduleData.assignedEmployees || []);
...
const displayBreaks = (breaksArray) => {
  ...
  return breaksArray.map((b) => `${b.startTime} - ${b.endTime}`).join(', ');
};
...
const saveSchedule = () => {
  const updatedSchedule = {
    ...
    weeklySchedule: weeklySchedule.value.map((day) => {
      // Clean up the data before emitting
      const dayData = { ...day };
      if (dayData.scheduleType === 'Day Off') {
        dayData.clockIn = '';
        dayData.clockOut = '';
        dayData.breaks = [];
      }
  ...
  (newVal) => {
    newVal.forEach((day) => {
      if (day.scheduleType === 'Day Off') {
        ...
        day.clockOut = '';
        day.breaks = [];
      } else if (!day.breaks || day.breaks.length === 0) {
        day.breaks = [
          { type: 'Short Break 1', startTime: '', endTime: '' },
          { type: 'Lunch Break', startTime: '', endTime: '' },
          { type: 'Short Break 2', startTime: '', endTime: '' },
        ];
      }
    });
  },
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <h2 class="text-2xl font-bold dark:text-white">Edit Weekly Schedule</h2>
        <button
          @click="close"
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-4 py-2">
                    <input
                      type="text"
                      :value="displayBreaks(daySchedule.breaks)"
                      :disabled="daySchedule.scheduleType === 'Day Off'"
                      class="w-full rounded-xl shadow-inner border border-gray-300 dark:border-gray-600 dark:bg-gray-700 bg-white dark:text-white text-gray-700 px-2 py-1 outline-none"
                      readonly
                    />
        ...
        <div class="mb-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div class="relative" v-click-outside="() => (showLocationDropdown = false)">
              ...
              >
                <span v-if="selectedLocations.length === 0"
                  >Select locations...</span
                >
                <span v-else-if="selectedLocations.length === locations.length"
                  >All Locations</span
                >
            ...
            <div
              class="relative"
              v-click-outside="() => (showDepartmentDropdown = false)"
            >
              <label
                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
              ...
              >
                ...
                >
                <span
                  v-else-if="selectedDepartments.length === departments.length"
            ...
            <div
              class="relative"
              v-click-outside="() => (showPositionDropdown = false)"
            >
              <label
                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
              ...
              <button
                ...
                class="w-full rounded-xl shadow-inner border border-gray-300 dark:border-gray-600 dark:bg-gray-700 bg-white dark:text-white text-gray-700 px-4 py-2 text-left flex justify-between items-center"
              >
                <span v-if="selectedPositions.length === 0"
                  >Select positions...</span
                >
                <span v-else-if="selectedPositions.length === positions.length"
                  >All Positions</span
                >
