...
const props = defineProps({
  ...
  initialColumns: {
    type: Array,
    default: () => ['Client Name', 'Industry', 'Contact', 'Status', 'Actions']
  }
});
...
<template>
  <Transition name="modal-fade">
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 backdrop-blur-sm" @click="closeModal"></div>
      <div class="relative bg-white dark:bg-darkBlue rounded-xl shadow-2xl p-8 w-full max-w-xl border dark:border-lightBlue border-clientPurple">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-2xl font-bold dark:text-white text-gray-800">Customize Clients Layout</h2>
          ...
          <div class="flex flex-wrap items-center gap-2 p-2 border dark:border-gray-600 border-gray-400 rounded-md bg-gray-50 dark:bg-navBlue min-h-[40px]">
            ...
            <div class="relative flex-grow min-w-[120px]">
              ...
              >
                ...
                <option v-for="col in columnsAvailableToAdd" :key="col" :value="col">{{ col }}</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div class="mt-8 flex justify-end gap-3">
          <button @click="cancelLayout"
            class="px-6 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600">
            CANCEL
          </button>
          <button @click="saveLayout"
            class="px-6 py-2 rounded-md bg-lightGreen text-white hover:bg-green-600">
            SAVE LAYOUT
          </button>
        </div>
      </div>
